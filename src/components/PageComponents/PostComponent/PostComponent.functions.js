...
export function EditPost({ isEditing, setIsEditing, content, setLoading, loading, postId }) {
...
export function RepostModal({ setIsModalOpenRepost, postId, isModalOpenRepost }) {
  ...
  return (
    ...
    >
      <p style={{
        ...
        this link?</p>
      <div style={{ display: "flex" }} >
        <button onClick={() => setIsModalOpenRepost(false)}
          data-test="cancel"
          style={{
            marginRight: "10px",
            width: "134px",
...
export function LikesTooltip(likes, isLiked, likedUserIds, userId, numberLikes) {
  if (likes.length === 0) {
...
export function PostOwner({ setIsEditing, isEditing, idUser, userId, setIsModalOpen }) {
  return (<>
    {Number(userId) === idUser ? (
      <div>
        <styles.StyledPencil data-test="edit-btn" onClick={() => setIsEditing(!isEditing)} />
        <styles.StyledTrash data-test="delete-btn" onClick={() => setIsModalOpen(true)} />
      </div>
    ) : (
      ""
    )}</>)
